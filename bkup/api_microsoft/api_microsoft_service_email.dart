...
import '../../utils/api/http_header.dart';
...
import '../tiki_http/http_client.dart';
import '../tiki_http/model/http_request.dart';
import '../tiki_http/model/http_verb.dart';
...
class ApiMicrosoftServiceEmail extends DataFetchInterfaceEmail {
  ...
  Future<void> fetchInbox(ApiOAuthModelAccount account,
    ...
    List<TikiHttpRequest> requests = List<TikiHttpRequest>.empty(
        growable: true); //TODO do we really need this?
    for (int i = 0; i < 1; i++) {
      //TODO this can't be right
      int currentPage = pageNum + i;
      String query = _buildQuery(page: currentPage, maxResults: 1);
      ...
      TikiHttpRequest tikiHttpRequest = TikiHttpRequest(
          ...
          map: HelperApiHeaders(auth: account.accessToken).header);
      tikiHttpRequest.onResult = (rsp) async {
        _log.finest(
            'Fetch inbox ${account.username} $currentPage onSuccess callback with ${rsp.statusCode} code.');
        if (TikiHttpClient.isTooManyRequests(rsp.statusCode)) {
          int retry = int.parse(rsp.map.entries
        ...
        if (TikiHttpClient.is2xx(rsp.statusCode)) {
          ...
          if (messageList.isEmpty) {
            ...
            List<DataFetchModelMsg> messages = messageList
                ...
                .map((message) => DataFetchModelMsg(
                    extMessageId: message['id'], account: account.email))
                .toList();
            int? next =
                msgBody['@odata.nextLink'] != null ? currentPage + 1 : null;
            DataFetchModelPage data =
                DataFetchModelPage(data: messages, next: next.toString());
            onResult(data);
          }
        }
        if (TikiHttpClient.isUnauthorized(rsp.statusCode)) {
          ...
          tikiHttpRequest.map =
              HelperApiHeaders(auth: account.accessToken).header;
      ...
      tikiHttpRequest.onError((error) {
        _log.finest(
            'Fetch inbox ${account.username} $currentPage onError callback.');
        _log.warning(error);
      });
