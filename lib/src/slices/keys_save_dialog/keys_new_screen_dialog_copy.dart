...
class KeysNewScreenDialogCopy {
  ...
  late KeysSaveScreenService keysService;
      ...
      Container(
        ...
        child: Column(
          children: [
            ...
            KeysNewScreenDialogCopyButton(currentModel.email, keysService,
                () => _copyEmail(currentModel.email!)),
            Padding(
                padding: EdgeInsets.symmetric(vertical: _marginVerticalField)),
            KeysNewScreenDialogCopyButton(
                combinedKey, keysService, () => _copyKey(combinedKey)),
          ],
        ),
      ),
      Container(
          padding: EdgeInsets.only(bottom: _marginBottomButton),
          child: TikiBigButton('CONTINUE', true,
              (context) => Navigator.of(context).pop(canContinue)))
    ]));
  }

  _copyEmail(String email) async {
    keysService.controller.copyEmail(keysService, email);
  }

  _copyKey(String combinedKey) {
    keysService.controller.copyEmail(keysService, combinedKey);
  }
}
